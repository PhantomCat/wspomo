...
function createStorage({ databaseUrl, logger = console }) {
  ...
  async function init() {
    ...
    // pg_advisory_xact_lock serializes concurrent inits: node --test runs each
    // test file in its own process, and two parallel CREATE TABLE IF NOT EXISTS
    // against a fresh database race on the pg_type catalog (duplicate key).
  ...
  // ---------- active timer session (server-authoritative chains, 21.09) ----------
  // One row per user (the timer chain is singular). started_at is metadata
  // for when the user activated the chain; the replay math itself is pure
  // schedule calculation (timer-core), anchored to the workday grid.

  async function startActiveSession(userId, mode) {
    await pool.query(
      `INSERT INTO active_sessions (user_id, mode, started_at) VALUES ($1, $2, now())
       ON CONFLICT (user_id) DO UPDATE SET mode = $2, started_at = now()`,
      [userId, mode]
    );
  }

  async function stopActiveSession(userId) {
    const r = await pool.query('DELETE FROM active_sessions WHERE user_id = $1', [userId]);
    return r.rowCount > 0;
  }

  async function getActiveSession(userId) {
    const r = await pool.query('SELECT user_id, mode, started_at FROM active_sessions WHERE user_id = $1', [userId]);
    return r.rows[0] || null;
  }
